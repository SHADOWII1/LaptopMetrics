...
- [Table of Contents](#table-of-contents)
  ...
  - [2.  Part II: The development of a Cloud Native Application \[Prof. Dr.-Ing. habil. Dennis Pfisterer\]](#2--part-ii-the-development-of-a-cloud-native-application-prof-dr-ing-habil-dennis-pfisterer)
    ...
    - [2.3  Implementation](#23--implementation)
      - [2.3.1  Project Structure](#231--project-structure)
      - [2.3.2  Virtual Machine Provision and Configuration](#232--virtual-machine-provision-and-configuration)
        - [2.3.2.1  Virtual Machine Provision with Terraform](#2321--virtual-machine-provision-with-terraform)
        - [2.3.2.2  Virtual Machine Configuration with Ansible](#2322--virtual-machine-configuration-with-ansible)
        - [2.3.2.3  CI/CD with Github Actions](#2323--cicd-with-github-actions)
      - [2.3.3  Monitoring and Visualization](#233--monitoring-and-visualization)
        - [2.3.3.1  Monitoring with Prometheus](#2331--monitoring-with-prometheus)
        - [2.3.3.2  Visualization with Grafana](#2332--visualization-with-grafana)
...
Over the last couple of years, the term "Cloud Native" has become very popular among software developers and in circles dealing with IT infrastructure. As organizations have undergone progressive digital transformation, cloud-native applications have become an essential component of modern IT strategies. The term "cloud-native" refers to the design, development, and deployment of applications specifically intended to exploit the scalability, resilience, and flexibility provided by the cloud. Overall, cloud-native applications are normally based on microservices architectures, managed by container orchestration systems like Kubernetes, and deployed into environments that support methodologies for continuous integration and continuous delivery. The move toward cloud-native development offers many benefits, specifically for applications that require high scalability, high availability, or deal with vast amounts of data. However, like any technology solution, cloud-native approaches bring their own set of problems and drawbacks. Moreover, other strategies, including traditional monolithic architectures or hybrid cloud models, have their strengths but also limitations, which must be carefully considered in choosing the best approach for an application.

Cloud-native architectures have revolutionized application development, deployment, and management, by offering flexibility, scalability, and resilience. These architectures allowed *LaptopMetrics*, An application designed to monitor the performance of a host system, to benefit from the aforementioned benefits.
...
3. **Vendor Lock-In**  
   Vendor lock-in is a potential risk for `LaptopMetrics`, particularly if the application relies heavily on specific cloud provider services. This reliance on a single vendor could limit the application's flexibility and make it difficult to take advantage of more competitive offerings from other providers.
...
1. **Encryption**
   Encryption is to be used both at rest and in transit. Performance metrics collected from the host are to be encrypted before being transmitted to the cloud, ensuring that data cannot be intercepted or tampered with. Similarly, data stored in cloud databases is to be encrypted at rest, protecting it from unauthorized access even if the storage medium is compromised.
...
### 2.1 <a name='Objective'></a> Objective

The goal of this project is to manage every phase, from architectural planning to design and implementation, using a cloud-native approach. The application, named `LaptopMetrics`, will be developed in C# as a monitoring solution. However, the primary focus will be on ensuring a fully cloud-native architecture with seamless deployment and scalability.

Throughout both development and deployment, the project will leverage the full potential of cloud technologies. A key priority will be high-level automation, whether in provisioning cloud infrastructure or in streamlining its configuration. Additionally, critical aspects such as monitoring and local development within Kubernetes will be explored in depth. Ultimately, the application will embody the core principles of a cloud-native solution and be designed to handle the entire lifecycle efficiently in the cloud environment.

### 2.2 <a name='architecture'></a> Architecture

`LaptopMetrics` is a C# web application developed using ASP.NET Core v8.0. The application's purpose is to run on the host system and gather real-time data on the state of the CPU, RAM, storage, and network connectivity. Instead of developing a frontend to visualize the retrieved data, a decision was made to use Grafana for this purpose. The backend is supported by MyAPI, a REST API that provides a Swagger UI for developers to test the application and access all endpoints. The application offers one endpoint, with a POST operation to expose the scraped data.

| ![Architecture.svg](img/Architecture.svg) | 
|:--:| 
| *Architecture Overview* |

In addition to the REST API handling the backend, Prometheus libraries for .NET applications are also included. On one hand, these libraries allow developers to use predefined functions to append the `/metrics` endpoint with the necessary data, formatted in a way that Prometheus can easily target. On the other hand, this setup enables the monitoring of the application; the Prometheus dependency injects the application with the necessary metrics, which can then be scraped by Prometheus.
...
The virtual machine hosted in the cloud is divided into two environments: a `Docker environment` and a `Kubernetes environment`. Each environment offers a range of benefits, and the aim is to leverage the strengths of each one.
...
| **Advantage**              | **Description**                                                                                                                                                     |
|----------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Scalability**            | Kubernetes supports advanced scaling capabilities, which is critical for handling high traffic in `LaptopMetrics`.                                     |
| **Load Balancing**         | Automatically manages load distribution across the cluster, ensuring that the application remains responsive under varying loads.                                    |
| **High Availability**      | Kubernetes provides redundancy and automatic failover, which enhances the reliability and uptime of `LaptopMetrics`.                                                |
...
| **Advanced Orchestration** | Facilitates complex tasks like rolling updates, self-healing, and automatic scaling, making it ideal for managing `LaptopMetrics` in production.                    |
...
Internally, Prometheus can scrape the data exposed by Grafana, LaptopMetrics, and itself using the name of each service and the assigned internal port.
...
#### 2.3.1 <a name='project-structure'></a> Project Structure
...
| **LaptopMetrics**    | Contains the source code for LaptopMetrics application.                                                                                                                 |
...
#### 2.3.2 <a name='virtual-machine-setup-and-configuration'></a> Virtual Machine Provision and Configuration

##### 2.3.2.1 <a name='virtual-machine-provision-terraform'></a> Virtual Machine Provision with Terraform
...
##### 2.3.2.2 <a name='virtual-machine-configuration-ansible'></a> Virtual Machine Configuration with Ansible
...
##### 2.3.2.3 <a name='ci-cd-with-github-actions'></a> CI/CD with Github Actions
...
#### 2.3.3 <a name='monitoring-and-visualization'></a> Monitoring and Visualization

##### 2.3.3.1 <a name='monitoring-with-prometheus'></a> Monitoring with Prometheus
...
##### 2.3.3.2 <a name='visulatization-with-grafana'></a> Visualization with Grafana
